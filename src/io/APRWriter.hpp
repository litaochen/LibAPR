...
public:
   ...
   */
    template<typename ImageType>
    FileSizeInfo write_apr_alt(APR<ImageType> &apr, const std::string &save_loc, const std::string &file_name, APRCompress<ImageType> &apr_compressor, unsigned int blosc_comp_type = BLOSC_ZSTD, unsigned int blosc_comp_level = 2, unsigned int blosc_shuffle=1) {
        APRTimer write_timer;
        ...
        unsigned int blosc_comp_level_ad = 6;

        for (int j = 0; j < map_data.y_end.size(); ++j) {
            map_data.y_end[j] = map_data.y_end[j] - map_data.y_begin[j];
        }

        writeData(AprTypes::MapYendType, f.objectId, map_data.y_end, blosc_comp_type, blosc_comp_level_ad, blosc_shuffle);
        writeData(AprTypes::MapYbeginType, f.objectId, map_data.y_begin, blosc_comp_type, blosc_comp_level_ad, blosc_shuffle);
        writeData(AprTypes::MapNumberGapsType, f.objectId, map_data.number_gaps, blosc_comp_type, blosc_comp_level_ad, blosc_shuffle);
        writeData(AprTypes::MapLevelType, f.objectId, map_data.level, blosc_comp_type, blosc_comp_level_ad, blosc_shuffle);
        writeData(AprTypes::MapXType, f.objectId, map_data.x, blosc_comp_type, blosc_comp_level_ad, blosc_shuffle);
        writeData(AprTypes::MapZType, f.objectId, map_data.z, blosc_comp_type, blosc_comp_level_ad, blosc_shuffle);
        //writeData(AprTypes::ParticleCellType, f.objectId, apr.apr_access.particle_cell_type.data, blosc_comp_type, blosc_comp_level, blosc_shuffle);
        write_timer.stop_timer();
