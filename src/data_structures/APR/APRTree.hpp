...
/*
    ...
    template<typename T,typename S,typename U>
    static void fill_multi_tree_mean_py(APR<T>& apr,APRTree<T>& apr_tree, py::array& particle_data,std::vector<ExtraParticleData<U>>& tree_data) {
        ...
        for(int i=0; i<tree_data.size(); ++i) {
            tree_data[i].init(apr_tree.total_number_parent_cells());
        }
        ...
        int in_channels = tree_data.size();
        py::buffer_info particle_buf = particle_data.request();
        auto particle_ptr = (float *)particle_buf.ptr;
                        ...
                        for (int x = 2 * x_d; x <= std::min(2 * x_d + 1, (int) apr.spatial_index_x_max(level)-1); ++x) {
                            ...
                            for (apr_iterator.set_new_lzx(level, z, x);
                                ...
                                if (parentIterator.y() == (parentIterator.spatial_index_y_max(level - 1) - 1)) {
                                    ...
                                    int offset = apr_iterator.global_index()*in_channels;

                                    for(int tr=0; tr<tree_data.size(); ++tr) {
                                        tree_data[tr][parentIterator] =
                                                scale_factor_yxz * particle_ptr[offset + tr] / 8.0f +
                                                tree_data[tr][parentIterator];
                                    }
                                } else {
                                    ...
                                    for(int tr=0; tr<tree_data.size(); ++tr) {
                                        tree_data[tr][parentIterator] =
                                                scale_factor_xz * particle_ptr[offset + tr] / 8.0f +
                                                tree_data[tr][parentIterator];
                                    }
                                }
                            }
