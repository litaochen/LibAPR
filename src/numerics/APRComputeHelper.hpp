...
#pragma omp parallel for schedule(static) private(particle_number) firstprivate(apr_iterator)
    ...
    void compute_local_scale_smooth_propogate(APR<ImageType>& apr,PixelData<ImageType>& input_image,ExtraParticleData<ImageType>& local_intensity_scale){
    ...
    void compute_local_min_max_apr(APRConverter<ImageType>& apr_converter,APR<ImageType>& apr,APR<ImageType>& apr_new){
        ...
        PixelData<uint16_t> input_img= TiffUtils::getMesh<uint16_t>(inputTiff);
        ...
        * */
        ...
        PixelData<uint16_t> custom_local_scale;
        PixelData<uint16_t> custom_grad; //if it is not initialized, the classic approach to using the gradient will be used.
